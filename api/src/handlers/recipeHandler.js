...
const { getAllRecipes, getRecipesById, createRecipe } = require('../controllers/recipesControllers');
...
const recipeByIdHandler = async (req, res) => {
    ...
    try {
        ...
        const result = source === "foodApi" ? await axios.get(url) : await getRecipesById(id);
...
const apiRecipesMap = (array) => 
    ...
    // toLowerCase o regex?? en includes
const getAllApiRecipes = async (name) => {
    ...
    if(name) apiRecipesArray = apiRecipesArray.filter(recipe => recipe.title.toLowerCase().includes(name.toLowerCase()));
